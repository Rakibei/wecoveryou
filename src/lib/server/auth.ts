...
export async function validateSessionToken(token: string) {
	...
	const [result] = await db
		...
		.from(table.session)
		.innerJoin(table.user, eq(table.session.userId, table.user.guid))
	...
	if (session.rememberMe === true && renewSession) {
...
export async function invalidateSession(sessionId: string) {
	await db.delete(table.session).where(eq(table.session.userId, sessionId));
}
